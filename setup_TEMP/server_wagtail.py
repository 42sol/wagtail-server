...
from www42sol.wsgi import application
...
def main( x_s_app_path = '/' ):
    ...
    # -[4] setup apps
    setup_app(l_o_server, 'web42sol', 8001) # use default parameters for host ip-address (local host)
    
    l_o_server2 = cherrypy._cpserver.Server()
    setup_app(l_o_server2, 'web4elsungen', 8002) # use default parameters for host ip-address (local host)
    
    l_o_server3 = cherrypy._cpserver.Server()
    setup_app(l_o_server3, 'web4sandrahaeberle', 8003) # use default parameters for host ip-address (local host)
